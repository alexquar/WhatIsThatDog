...
import { StyleSheet, View, Text, SafeAreaView, Image, TouchableOpacity, ScrollView, Alert, RefreshControl } from 'react-native';
...
const Feed = () => {
  ...
  const fetchPosts = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://10.0.0.64:5000/posts');
  ...
  const handleLikePost = async (postId: string) => {
    ...
    try {
      const response = await fetch(`http://10.0.0.64:5000/post/${postId}`, {
        method: 'PUT',
        headers: {
      ...
      Alert.alert('Error', 'Failed to like that fuzzy friend. Please try again later.');
    }
  };
      ...
      <ScrollView
        ...
        <View key={post._id} style={styles.postCard}>
          ...
          <View style={{ position: 'relative' }}>
            <Image
          source={post?.file ? { uri: post.file } : require('../../assets/images/logo.png')}
